...
const createMessageHandler = async (req, res) => {
  try {
    ...
    try {
      ...
      await pipeline(ollamaReadable, tokenTransform, res);
      ...
      if (assistantResponse && !streamAborted) {
        ...
        // Update chat's updatedAt timestamp to mark it as recently active
        ...
        // Update chat's updatedAt timestamp even for partial responses
        await prisma.chat.update({
          where: { id: chatId },
          data: { updatedAt: new Date() },
        });
      ...
      // Don't send error message to client - let frontend handle with retry button
      // Just end the stream so frontend knows there was an error
...
const stopChatHandler = async (req, res) => {
  try {
    ...
    console.log(
      "Stop request received for chatId:",
      chatId,
      "streamId:",
      streamId
    );
